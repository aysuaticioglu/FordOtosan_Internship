...
def image_mask_check(image_path_list, mask_path_list):
    ...
    image_names = [os.path.splitext(os.path.basename(image_path))[0] for image_path in image_path_list]
    mask_names = [os.path.splitext(os.path.basename(mask_path))[0] for mask_path in mask_path_list]
    ...
    for image_name, mask_name in zip(image_names, mask_names):
        if image_name != mask_name:
            ...
            print("Images and masks folder should have the same file names.")
            return False
...
def one_hot_encoder(data, n_class):
    ...
    encoding_labels = np.unique(data)
    ...
    #
    for lbl, encoded_labels in enumerate(np.unique(data)):
       
        encoded_data[:, :, lbl][data == encoded_labels] = 1
